...
class BMMDCM(BMMOpticsBase):
    ...
    def energy_to_bragg(self, energy_ev):
        ...
        d_spacing = (
            self.d_spacing_111
            if self.current_reflection == "111"
        ...
        if sin_theta > 1:
            raise ValueError(
                f"Energy {energy_ev} eV not achievable with {self.current_reflection} reflection"
            )
    ...
    def set_energy(self, energy_ev):
        ...
        try:
            ...
            logger.info(
                f"Moving {self.name} to {energy_ev} eV (Bragg: {bragg_angle:.4f}°)"
            )

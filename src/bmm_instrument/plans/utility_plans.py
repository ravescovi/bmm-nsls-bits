...
import logging
import time
from bluesky.plan_stubs import sleep, mv, mvr, trigger_and_read, null
    ...
    for energy in test_energies:
        ...
        try:
            ...
            if hasattr(dcm, 'set_energy'):
                ...
                logger.warning(f"Cannot set energy - no suitable method")
                continue
    ...
    for controller in temp_controllers:
        ...
        if hasattr(controller, 'temperature'):
            try:
                ...
                logger.warning(f"Could not read initial temperature from {controller.name}")
...
def safe_shutdown_sequence(*devices, md=None):
    ...
    *devices : ophyd.Device
    ...
    temp_controllers = [dev for dev in devices if 'temp' in dev.name.lower() or 'lakeshore' in dev.name.lower()]
    for controller in temp_controllers:
        try:
    ...
    motors = [dev for dev in devices if hasattr(dev, 'position') and hasattr(dev, 'move')]
    for motor in motors:
        try:

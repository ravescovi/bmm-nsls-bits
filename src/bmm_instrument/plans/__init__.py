...
BASIC_PLANS = [
    'move', 'mover', 'multi_move', 'multi_move_relative', 'sleep_plan',
    'count_plan', 'motor_scan_plan', 'safe_move', 'wait_for_temperature',
    'motor_status_check', 'mv_plan', 'mvr_plan', 'kmv', 'kmvr'
]

XAFS_PLANS = [
    'xafs_scan', 'xafs_step_scan', 'transmission_xafs', 'fluorescence_xafs',
    'quick_xafs', 'xafs_with_temperature', 'energy_calibration_scan',
    'copper_xafs', 'iron_xafs', 'zinc_xafs'
]

SCANNING_PLANS = [
    'line_scan', 'relative_line_scan', 'area_scan', 'time_scan', 'fly_scan',
    'spiral_scan', 'raster_scan', 'multi_motor_scan', 'adaptive_scan',
    'quick_scan', 'coarse_scan', 'fine_scan'
]

ALIGNMENT_PLANS = [
    'tune_dcm_pitch', 'align_slits', 'mirror_alignment', 'sample_height_scan',
    'beam_size_measurement', 'find_sample_edge', 'center_sample_on_beam',
    'energy_calibration_check', 'quick_mirror_tune', 'quick_slit_center'
]

UTILITY_PLANS = [
    'motor_recovery_plan', 'detector_status_check', 'beamline_status_summary',
    'energy_system_check', 'temperature_system_check', 'safe_shutdown_sequence',
    'warm_up_sequence', 'quick_status_check', 'emergency_stop_all_motors',
    'diagnose_motor_issues'
]

# BITS plans
BITS_PLANS = [
    'dm_kickoff_workflow', 'dm_list_processing_jobs', 'dm_submit_workflow_job',
    'sim_count_plan', 'sim_print_plan', 'sim_rel_scan_plan'
]

# All available plans
ALL_PLANS = BASIC_PLANS + XAFS_PLANS + SCANNING_PLANS + ALIGNMENT_PLANS + UTILITY_PLANS + BITS_PLANS
...
def list_plans(category=None):
    ...
    if category is None:
        ...
        raise ValueError(f"Unknown category: {category}. Available: {list(PLAN_CATEGORIES.keys())}")
